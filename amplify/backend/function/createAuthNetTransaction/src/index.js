...
function processorErrorMessage(response, transaction) {
  const detail = transaction?.getErrors?.()?.getError?.()?.[0];
  const fallback = response?.getMessages?.()?.getMessage?.()?.[0];
  const code = detail?.getErrorCode?.() || fallback?.getCode?.();
  const text = detail?.getErrorText?.() || fallback?.getText?.();
  return [code, text].filter(Boolean).join(': ') || 'Unknown processor response';
}
