...
## GraphQL Folder Structure (IMPORTANT)

**Two GraphQL folders exist intentionally:**

1. **`src/graphql/`** - Auto-generated by AWS Amplify
   - Contains: `queries.js`, `mutations.js`, `subscriptions.js`, `schema.json`
   - **DO NOT IMPORT** from this folder
   - **DO NOT DELETE** - Will regenerate automatically
   - Purpose: Amplify codegen output (not used in application)
   - Managed by: AWS Amplify CLI

2. **`src/queries/`** - Hand-written GraphQL operations (ACTIVE)
   - Contains: `queries.js`, `mutations.js`, `userQueries.js`, `userMutations.js`
   - **USE THIS FOLDER** for all GraphQL imports
   - Purpose: Application's actual GraphQL queries and mutations
   - Managed by: Development team

**When writing code:**
- Always import from `src/queries/`, never `src/graphql/`
- Do not attempt to consolidate or remove either folder
- If you see both folders, this is correct and intentional
